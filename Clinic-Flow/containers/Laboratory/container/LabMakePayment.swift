        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 25) {
                ...
                NavigationLink(destination: LabPaymentConfirmation()) {
                    Text("Make Payment").font(.headline).foregroundColor(.white).frame(maxWidth: .infinity).padding(.vertical, 18).background(Color.green).cornerRadius(30)
                }
            }
            .padding(25)
        }
